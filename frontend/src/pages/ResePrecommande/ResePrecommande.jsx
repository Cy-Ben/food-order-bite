...
const ResePrecommande = () => {
  ...
  const [step, setStep] = useState(null); // null | 'date' | 'heure' | 'menu' | 'confirm'
  ...
  // Charger les tables selon la zone
  useEffect(() => {
    if (zone) {
      setLoading(true);
  ...
  const confirmerTout = async () => {
    ...
    // Insérer les plats du panier si non vide
